...
MQTTClient::MQTTClient(int bufSize, int writeBufSize, int readBufSize, bool hasPSRAM)
{
  // allocate buffers
  this->hasPSRAM = hasPSRAM;
...
void MQTTClient::begin(Client &_client)
{
  ...
  // initialize client
  lwmqtt_init(&this->client, this->writeBufSize, this->readBufSize, hasPSRAM);
...
bool MQTTClient::publish(const char topic[], uint8_t *payload, int length, bool retained, int qos)
{
...
bool MQTTClient::publishFile(const char topic[], File file, bool retained, int qos)
{
  // return immediately if not connected
  if (!this->connected())
  {
  ...
  message.payload_len = file_len;
  message.retained = retained;
  message.qos = lwmqtt_qos_t(qos);

  // publish message
  uint8_t *ptr_buf = lwmqtt_publish_part1(&this->client, lwmqtt_string(topic), message, this->timeout);
  if (ptr_buf == NULL)
  {
    Serial.println("erro parte 1");
    // close connection
    this->close();

    return false;
  }
  if (!file.read(ptr_buf, file_len))
  {
    Serial.println("Fail to read file to buf");
    return false;
  }

  size_t len = (ptr_buf + file_len) - (this->client.write_buf);
  Serial.println(len);
  this->_lastError = lwmqtt_publish_part2(&this->client, lwmqtt_string(topic), len, message, this->timeout);
  if (this->_lastError != LWMQTT_SUCCESS)
  {
    Serial.println("erro parte 2");
    return false;
  }
