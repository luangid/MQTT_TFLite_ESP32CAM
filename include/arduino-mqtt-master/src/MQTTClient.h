...
public:
  ...
  explicit MQTTClient(int bufSize, int writeBufSize, int readBufSize, bool hasPSRAM);
  ...
  bool publish(const char topic[], uint8_t *payload, int length, bool retained, int qos);

  bool publishFile(const char topic[], File file, bool retained, int qos);
