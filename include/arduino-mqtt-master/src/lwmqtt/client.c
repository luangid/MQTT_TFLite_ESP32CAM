...
void lwmqtt_init(lwmqtt_client_t *client, size_t write_buf_size, size_t read_buf_size, bool hasPSRAM)
{
  ...
  if (hasPSRAM)
  {
    client->write_buf = (uint8_t *)ps_malloc(write_buf_size);
    client->write_buf_size = write_buf_size;
    client->read_buf = (uint8_t *)ps_malloc(read_buf_size);
    client->read_buf_size = read_buf_size;
  }
  else
  {
    client->write_buf = (uint8_t *)malloc(write_buf_size);
    client->write_buf_size = write_buf_size;
    client->read_buf = (uint8_t *)malloc(read_buf_size);
    client->read_buf_size = read_buf_size;
  }
...
uint8_t *lwmqtt_publish_part1(lwmqtt_client_t *client, lwmqtt_string_t topic, lwmqtt_message_t message,
                              uint32_t timeout)
{
  // set command timer
  client->timer_set(client->command_timer, timeout);
  ...
  // encode publish packet
  size_t len = 0;
  uint8_t *ptr = lwmqtt_encode_only_header(client->write_buf, client->write_buf_size, &len, 0, packet_id, topic, message);

  return ptr;
}
lwmqtt_err_t lwmqtt_publish_part2(lwmqtt_client_t *client, lwmqtt_string_t topic, size_t len, lwmqtt_message_t message,
                                  uint32_t timeout)
{
